...
final class PlayerViewModel: ObservableObject {
    ...
    init() {
        ...
        systemMediaController.setHandlers(
            play: { [weak self] in self?.playFromSystemCommand() },
            pause: { [weak self] in self?.pauseFromSystemCommand() },
    ...
    private func playFromSystemCommand() {
        switch playbackState {
        case .playing:
            updateSystemNowPlayingInfo()
        case .paused:
    ...
    private func pauseFromSystemCommand() {
    ...
    private func updateSystemNowPlayingInfo() {
        guard let track = currentTrack, playbackState != .idle else {
            systemMediaController.update(snapshot: nil)
            return
        }

        systemMediaController.update(
            snapshot: SystemMediaController.PlaybackSnapshot(
                title: track.title,
                artist: track.artist,
                album: track.album,
                duration: track.duration > 0 ? track.duration : audioEngine.duration,
