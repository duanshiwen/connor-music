...
- ⏯️ 播放 / 暂停 / 上一曲 / 下一曲
- 🖥️ 接入 macOS Now Playing / 系统媒体控制，可通过系统媒体键或控制中心控制播放
- ⌨️ 窗口激活时支持空格播放 / 暂停；搜索框等文本输入场景不会拦截空格
- 🔁 一个按钮循环切换播放模式：顺序播放 → 随机播放 → 全部循环 → 单曲循环
- 🖼️ 自动读取专辑封面和音频元数据，支持读取 FLAC / Vorbis Comment 标签
- 🎯 当前播放曲目离开可视区域时，自动显示低调的浮动定位按钮，一键滚动回当前曲目
...
- 调低“定位当前播放曲目”浮动按钮视觉权重，改为更接近 macOS 原生的半透明材质按钮
...
2. 选择包含音频文件的文件夹
3. 双击曲目播放，或使用底部控制栏
4. 应用窗口激活时，可按空格切换播放 / 暂停
5. 可使用键盘媒体键或 macOS 控制中心控制播放
6. 点击播放列表表头可按标题、艺术家、专辑、时长排序
7. 向音乐文件夹添加新文件后，新文件会自动出现在播放列表中
...
├── SystemMediaController.swift # macOS Now Playing 与系统媒体命令
├── SpaceKeyHandler.swift  # 窗口级空格键播放 / 暂停处理
